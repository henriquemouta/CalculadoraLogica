...
namespace ProvaLogica
{
    ...
    public static class AnalisadorSintatico
    {
        ...
        private static bool podeSerInicio(string valor)
        {
            ...
            if (OperadoresUnarios.Contains(valor)) return true;
            if (ProposicaoRegex.IsMatch(valor)) return true;
            return false;
        }
        ...
        private static bool ehProposicao(string v) => ProposicaoRegex.IsMatch(v);
        private static bool transicaoValida(string atual, string proximo)
        {
            ...
            if (atual == "(")
            {
                ...
                if (ehOperadorUnario(proximo)) return true;
                if (ehProposicao(proximo)) return true;
                return false;
            }

            if (ehOperadorUnario(atual))
            {
                if (proximo == "(") return true;
                if (ehOperadorUnario(proximo)) return true;
                if (ehProposicao(proximo)) return true;
            ...
            if (ehProposicao(atual))
            {
                if (ehOperadorBinario(proximo)) return true;
                if (proximo == ")") return true;
                return false;
            }

            if (ehOperadorBinario(atual))
            {
                if (proximo == "(") return true;
                if (ehOperadorUnario(proximo)) return true;
                if (ehProposicao(proximo)) return true;
    ...
    public static class GeradorTabelaVerdade
    {
        public static void gerar(List<Simbolo> simbolos)
        {
            ...
            foreach (var c in colunas)
                Console.Write(c.PadRight(larguraColuna));
            Console.WriteLine();
            ...
            for (int mask = 0; mask < (1 << proposicoes.Count); mask++)
            {
                ...
                if (resultados[colunas.Last()]) todosFalsos = false;
                else todosVerdadeiros = false;
            }
    ...
    public class Programa
    {
        public static void Main()
        {
            ...
            Console.WriteLine("Use: ~ (negação), & (e), | (ou), -> (condicional), <-> (bicondicional)");
            Console.WriteLine("Exemplo: (A->B)<->(~B->~A)");
            Console.WriteLine();

            while (true)
            {
                Console.Write("Digite a fórmula proposicional (ou 'sair'): ");
                ...
                try
                {
                    ...
                    if (!AnalisadorSintatico.formulaBemFormada(simbolos))
                    {
                        ...
                        Console.WriteLine("Erro: Fórmula não é bem formada.");
                        Console.ResetColor();
                        continue;
                    }
